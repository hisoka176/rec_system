...
def input_fn(mode, params):
    file_pattern = params['dataset']['train_dataset'] if mode == tf.estimator.ModeKeys.TRAIN else params['dataset'][
        'test_dataset']
    dataset = tf.data.experimental.make_csv_dataset(file_pattern=file_pattern, header=True, batch_size=16384,
                                                    prefetch_buffer_size=16384 * 2, num_parallel_reads=32)
